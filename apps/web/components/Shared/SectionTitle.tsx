...
const SectionTitle = ({title, children, link}: SectionTitlerops) => {
  return (
    <div className={`flex flex-col ${link ? 'gap-5 mt-12.5 lg:mt-25 2xl:mt-37.5 lg:flex-row lg:justify-between' : 'gap-1.5'}`}>
        <div className={`${link ? 'lg:max-w-[70%]' : ''}`}>
            <h1 className={`${link ? 'h1-title' : 'h1-sub-title text-grey-20'}`}>{title}</h1>
